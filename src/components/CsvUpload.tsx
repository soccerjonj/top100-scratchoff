...
export function CsvUpload({
  ...
  if (stage === "success") {
    return (
      <div className="flex flex-col gap-2 rounded-md border border-gold/40 bg-gold/10 p-4 text-sm">
        <div className="font-semibold text-gold">
          ✓ Imported {imported?.toLocaleString() ?? ""} films
        </div>
        <div className="text-xs text-zinc-300">
          You&apos;re done — from now on, new films you log on Letterboxd
          will show up here automatically. No re-upload needed.
        </div>
        <div className="text-xs text-zinc-400">Loading your grid…</div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <p className="text-[11px] text-zinc-500">
        <span className="text-gold">One-time setup.</span> After this we
        auto-check Letterboxd for new films on every visit — you won&apos;t
        have to re-upload.
      </p>
      {error && (
